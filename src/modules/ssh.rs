...
impl SSHModule {
    ...
    pub async fn connect(&mut self, index: usize) -> Result<()> {
        ...
        #[cfg(unix)]
        {
            ...
            let ssh_args: Vec<String> =
                vec!["ssh".into(), "-p".into(), port_s.clone(), target.clone()];
            let terminal_emulators: Vec<(&str, Vec<String>)> = vec![
                ("gnome-terminal", {
                    let mut v: Vec<String> = vec!["--".into()];
                    v.extend(ssh_args.clone());
                    v
                }),
                ("xterm", {
                    let mut v: Vec<String> = vec!["-e".into()];
                    v.extend(ssh_args.clone());
                    v
                }),
                ("konsole", {
        ...
        #[cfg(windows)]
        {
            ...
            if let Ok(child) = Command::new("cmd")
                .args(&["/C", "start", "ssh", "-p", &port_s, &target])
                .spawn()
            {
                session_pid = child.id().into();
            }
    ...
    pub fn add_from_string(&mut self, input: &str) -> Result<()> {
        ...
        if parts.len() < 2 {
            anyhow::bail!("Invalid format. Use: name|user@host:port");
        }
    ...
    pub fn refresh_session_status(&mut self) {
        ...
        for (name, host) in detected {
            if let Some(existing) = self
                .active_sessions
                .iter()
                .find(|s| s.name == name && s.host == host)
            {
                let mut s = existing.clone();
                s.status = "Connected".to_string();
                new_sessions.push(s);
            } else {
                new_sessions.push(SSHSession {
                    name: name.clone(),
                    host: host.clone(),
                    status: "Connected".to_string(),
                    connected_at: Local::now(),
