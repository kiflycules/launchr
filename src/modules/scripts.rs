...
impl From<ScriptConfig> for Script {
    fn from(config: ScriptConfig) -> Self {
        Self { name: config.name, command: config.command, description: config.description }
    }
}
...
impl ScriptsModule {
    pub fn new(config: &Config) -> Self {
        ...
        Self { scripts, running_scripts: HashSet::new(), config: config.clone() }
    }

    pub async fn run_script(&mut self, index: usize) -> Result<()> {
        if index >= self.scripts.len() { return Ok(()); }
        let script = &self.scripts[index];
        self.running_scripts.insert(index);

        let parts: Vec<String> = script.command.split_whitespace().map(|s| s.to_string()).collect();
    ...
    pub fn is_running(&self, index: usize) -> bool { self.running_scripts.contains(&index) }

    pub fn add_from_string(&mut self, input: &str) -> Result<()> {
        let parts: Vec<&str> = input.split('|').collect();
        if parts.len() < 2 { anyhow::bail!("Invalid format. Use: name|command"); }
        let name = parts[0].trim().to_string();
        let command = parts[1].trim().to_string();
        let description = if parts.len() > 2 { parts[2].trim().to_string() } else { String::new() };
        let script = Script { name: name.clone(), command: command.clone(), description: description.clone() };
        let config_script = ScriptConfig { name, command, description };
