...
impl DockerModule {
    ...
    pub fn refresh_containers(&mut self) -> Result<()> {
        ...
        let args = if self.show_all {
            vec!["ps", "-a", "--format", "{{.ID}}|{{.Names}}|{{.Image}}|{{.Status}}|{{.Ports}}|{{.CreatedAt}}"]
        } else {
            vec!["ps", "--format", "{{.ID}}|{{.Names}}|{{.Image}}|{{.Status}}|{{.Ports}}|{{.CreatedAt}}"]
        };
    ...
    pub fn start_container(&self, index: usize) -> Result<()> {
        if index >= self.containers.len() { return Ok(()); }
    ...
    pub fn exec_into_container(&self, index: usize) -> Result<()> {
        ...
        #[cfg(unix)]
        {
            ...
            let docker_cmd = format!("docker exec -it {} /bin/bash || docker exec -it {} /bin/sh", 
    ...
    pub fn view_logs(&self, index: usize) -> Result<String> {
        if index >= self.containers.len() { return Ok(String::new()); }
        let container = &self.containers[index];
